...
class IndexStrategy:
    ...
    def _apply_caps(self, weights):
        ...
        for iteration in range(15):  # Increased iterations for complex scenarios
            old_weights = weights.copy()
            
            # Step 1: Apply combined cap first
            weights = self._cap_btc_eth_combined(weights)
            
            # Step 2: Apply individual BTC/ETH caps
            weights = self._cap_btc(weights)
            weights = self._cap_eth(weights)
            
            # Step 3: Apply max_any cap LAST (enforces limit on ALL tokens including BTC/ETH)
    ...
    def _cap_btc(self, weights):
        if 'BTC' in weights and weights['BTC'] > self.btc_cap:
            ...
            if others:
                # If max_any cap exists, respect it during redistribution
                if self.max_any < 1.0:
                    # Only redistribute to tokens with headroom
                    others_with_room = {k: v for k, v in others.items() if v < self.max_any}
                    if others_with_room:
                        headroom = {k: self.max_any - v for k, v in others_with_room.items()}
                        total_headroom = sum(headroom.values())
                        if total_headroom > 0:
                            for k in others_with_room:
                                weights[k] += (headroom[k] / total_headroom) * excess
                    else:
                        # No headroom anywhere, distribute proportionally anyway
                        total = sum(others.values())
                        if total > 0:
                            for k in others:
                                weights[k] += (weights[k] / total) * excess
                else:
                    # No max_any cap, distribute proportionally
                    total = sum(others.values())
                    if total > 0:
                        for k in others:
                            weights[k] += (weights[k] / total) * excess
    ...
    def _cap_eth(self, weights):
        if 'ETH' in weights and weights['ETH'] > self.eth_cap:
            ...
            # Only redistribute to tokens that aren't ETH
                ...
                if others:
                    ...
                    if self.max_any < 1.0:
                        ...
                        if others_with_room:
                            ...
                            # No headroom, distribute proportionally anyway
                            total = sum(others.values())
                            if total > 0:
                                for k in others:
                                    weights[k] += (weights[k] / total) * excess
    ...
    def _cap_any_token(self, weights):
        """Cap any token to max_any, iteratively"""
        if self.max_any >= 1.0:
            return weights
        
        # Iterate until all tokens are under the cap
        for _ in range(20):
            capped = False
            for token, weight in list(weights.items()):
                if weight > self.max_any:
                    ...
                    # Redistribute only to tokens that have headroom under the cap
                    others = {k: v for k, v in weights.items() 
                             if k != token and v < self.max_any}
                    
                    if others:
                        # Calculate how much headroom each token has
                        headroom = {k: self.max_any - v for k, v in others.items()}
                        total_headroom = sum(headroom.values())
                        
                        if total_headroom > 0:
                            # Distribute proportionally to headroom
                            for k in others:
                                share = (headroom[k] / total_headroom) * excess
                                weights[k] += share
