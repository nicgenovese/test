    ...
    if s1_enabled:
        with st.sidebar.expander("Configure Strategy 1"):
            ...
            if s1_method == 'manual':
                ...
                top_10 = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'ADA', 'AVAX', 'DOT', 'MATIC', 'LINK']
                for token in top_10:
        ...
        with tab1:
            ...
            for r in results:
                fig.add_trace(go.Scatter(
                    ...
                    name=r['name'],
                    line=dict(color=r['color'], width=4),
                    mode='lines'
                ))
            ...
            for i, r in enumerate(results):
                ...
                <div style='background: white; padding: 2rem; border: 3px solid {r['color']}; 
                    ...
                    <div style='display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem;'>
                        ...
                        <div><strong>BTC Cap:</strong><br><span style='color: #1e40af; font-weight: 800;'>{c['btc_cap']*100:.0f}%</span></div>
                        <div><strong>ETH Cap:</strong><br><span style='color: #1e40af; font-weight: 800;'>{c['eth_cap']*100:.0f}%</span></div>
                        <div><strong>BTC+ETH:</strong><br><span style='color: #10b981; font-weight: 800;'>{c['btc_eth_combined']*100:.0f}%</span></div>
                        <div><strong>Max Any:</strong><br><span style='color: #f59e0b; font-weight: 800;'>{c['max_any']*100:.0f}%</span></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
